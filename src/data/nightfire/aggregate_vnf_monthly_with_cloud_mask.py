...
with RH / RHI additionally broken out by cloud-mask value.
...
For every calendar month it records:
  ...
  * per-cloud-mask RH/RHI  : RH and RHI summed within each cloud-mask value (detections only),
                             so e.g. rh_sum_cm0 pairs correctly with n_detect_cm0 for a
                             per-clear-look signal. These partition the top-level rh_sum/rhi_sum:
                             sum(rh_sum_cm0..cm3, rh_sum_cmNA) == rh_sum for every row.
...
# fields to additionally break out per cloud-mask value (detections only).
# For each, a per-mask sum is emitted. A per-mask n is emitted only where it is
# NOT already recoverable from n_detect_cm{v} (see note in aggregate_site).
PERMASK = ["RH", "RHI"]
# ------------------------------------------------------------------
...
def aggregate_site(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for m, idx in df.groupby("_month").groups.items():
        ...
        # --- RH / RHI summed per cloud-mask value (detections only) ---
        # Same detection subset (gd) as the intensity block, so these partition the
        # top-level rh_sum / rhi_sum exactly -- the mask buckets add back to the total.
        #
        # NOTE on the per-mask n: detection is defined as RH.notna(), so within gd every
        # RH is present and rh_n_cm{v} would just duplicate n_detect_cm{v}. We therefore
        # emit a per-mask n ONLY for columns where it is genuinely new -- i.e. RHI, which
        # can be NaN on rows where RH is present, making rhi_n_cm{v} the correct (and
        # otherwise unavailable) divisor for a per-clear-look RHI.
        for col in PERMASK:
            if col not in gd:
                continue
            lo = col.lower()
            cvals = gd[CLOUD_COL]
            emit_n = (col != RH_COL)   # skip n for the detection-defining column
            for v in CLOUD_VALUES:
                s_v = gd.loc[cvals == v, col].dropna()
                rec[f"{lo}_sum_cm{v}"] = float(s_v.sum())
                if emit_n:
                    rec[f"{lo}_n_cm{v}"] = int(s_v.size)
            s_na = gd.loc[cvals.isna(), col].dropna()
            rec[f"{lo}_sum_cmNA"] = float(s_na.sum())
            if emit_n:
                rec[f"{lo}_n_cmNA"] = int(s_na.size)
